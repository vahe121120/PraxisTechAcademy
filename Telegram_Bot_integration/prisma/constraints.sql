...
-- Course groups: end date, when set, must be after the start date.
ALTER TABLE "course_groups"
  ADD CONSTRAINT "chk_course_groups_end_after_start"
  CHECK ("end_date" IS NULL OR "end_date" >= "start_date");

-- Course groups: capacity, when set, must be positive.
ALTER TABLE "course_groups"
  ADD CONSTRAINT "chk_course_groups_capacity_positive"
  CHECK ("capacity" IS NULL OR "capacity" > 0);

-- Course groups: schedule must not be blank/whitespace-only — the column
-- is NOT NULL already, but that alone permits an empty string.
ALTER TABLE "course_groups"
  ADD CONSTRAINT "chk_course_groups_schedule_not_blank"
  CHECK (length(btrim("schedule")) > 0);

-- Course groups: the assigned teacher (when set) must actually hold the
-- TEACHER or ADMIN role. Postgres CHECK constraints can't reference another
-- table directly, so this is enforced with a trigger — the application
-- layer (CourseGroupsService) validates this too for a friendly error
-- message before the row is ever written; this trigger is the
-- can't-be-bypassed backstop for any write path that isn't the API (a
-- script, a manual psql session, a future admin tool).
CREATE OR REPLACE FUNCTION enforce_course_group_teacher_role()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.teacher_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION 'course_groups.teacher_id (%) must reference a user with role TEACHER or ADMIN', NEW.teacher_id;
    END IF;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_course_groups_teacher_role ON "course_groups";
CREATE TRIGGER trg_course_groups_teacher_role
  BEFORE INSERT OR UPDATE OF teacher_id ON "course_groups"
  FOR EACH ROW
  EXECUTE FUNCTION enforce_course_group_teacher_role();

-- Subscriptions: expireDate must be after startDate.
ALTER TABLE "subscriptions"
  ADD CONSTRAINT "chk_subscriptions_period_valid"
  CHECK (
    "start_date" IS NULL
    OR "expire_date" IS NULL
    OR "expire_date" > "start_date"
  );
