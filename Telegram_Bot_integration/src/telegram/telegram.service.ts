...
export class TelegramService implements TelegramAccessPort {
  ...
  async grantAccess(input: GrantAccessInput): Promise<void> {
    ...
    if (!telegramLink) {
      this.logger.warn(
        `Cannot grant Telegram access for user ${input.userId} — Telegram account not linked yet.`,
      );
    ...
    try {
      const grant = await this.createInviteLink(input.courseGroupId, input.userId);
      ...
      this.logger.error(
        `Failed to grant Telegram access for user ${input.userId} / course group ${input.courseGroupId}`,
        error instanceof Error ? error.stack : String(error),
      );
      await this.queueNotification(
        input.userId,
