...
async function bootstrap(): Promise<void> {
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    bufferLogs: true,
    // Preserves the exact raw request bytes on `req.rawBody` alongside the
    // normally-parsed body. Required by the ARCA webhook endpoint: a
    // re-serialized JSON body is not guaranteed to be byte-identical to
    // what the gateway actually signed, so signature verification must run
    // against these original bytes, not a reconstruction of them.
    rawBody: true,
  });
