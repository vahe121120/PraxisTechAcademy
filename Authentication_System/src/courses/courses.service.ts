...
export class CoursesService {
  ...
  async remove(id: string): Promise<void> {
    ...
    const cohortCount = await this.prisma.cohort.count({ where: { courseId: id } });
    if (cohortCount > 0) {
      // The database's own FK (Cohort.courseId, onDelete: Restrict) would
      // reject this anyway — this pre-check exists purely to give a much
      // clearer, actionable error than a raw P2003 conflict message.
      throw new ConflictException(
        `Cannot delete this course: it has ${cohortCount} cohort(s) referencing it. ` +
