...
export class SubscriptionsService {
  ...
  // ---------------------------------------------------------------------
  // Admin: manual activation ("Manually activate/deactivate access")
  // ---------------------------------------------------------------------

  /**
   * Comps or manually restores access without a real payment (e.g. a bank
   * transfer handled outside ARCA, a goodwill extension, correcting a
   * support issue). Deliberately reuses the exact same
   * `TELEGRAM_ACCESS_PORT.grantAccess()` call a successful payment
   * triggers (see `PaymentsService.applySuccessfulPayment`) rather than a
   * parallel "admin version" of granting access — there must be exactly
   * one code path that decides what "granting access" means.
   *
   * "Deactivate" has no separate method here: it's the existing `cancel()`
   * above, already admin-callable — see AdminController for why a
   * distinct admin-namespaced route still wraps it.
   */
  async manuallyActivate(
    ...
    if (subscription.status === SubscriptionStatus.CANCELLED) {
      throw new ConflictException(
        'This subscription was cancelled. Create a new enrollment instead of reactivating a cancelled one.',
      );
    }

    const resolvedExpireDate = expireDate ?? this.oneMonthFromNow();
    if (resolvedExpireDate.getTime() <= Date.now()) {
      throw new ConflictException('expireDate must be in the future.');
    }
    ...
    await this.recordAudit(actorId, 'SUBSCRIPTION_MANUALLY_ACTIVATED', updated.id, {
      status: SubscriptionStatus.ACTIVE,
      expireDate: resolvedExpireDate,
    });

    await this.safeGrantAccess({
      userId: updated.enrollment.userId,
      courseGroupId: updated.enrollment.courseGroup.id,
      subscriptionId: updated.id,
    });
  ...
  /** Same never-fail-the-caller contract as safeRevokeAccess — TelegramService.grantAccess already catches and records its own errors internally; this is the last-resort safety net. */
  private async safeGrantAccess(
    input: Parameters<TelegramAccessPort['grantAccess']>[0],
  ): Promise<void> {
    try {
      await this.telegramAccessPort.grantAccess(input);
    } catch (error) {
      this.logger.error(
        `Telegram access grant failed for subscription ${input.subscriptionId}`,
        error instanceof Error ? error.stack : String(error),
      );
    }
  }
    ...
    try {
      await this.prisma.notificationLog.create({
        data: {
          ...
          metadata: { subscriptionId } as unknown as Prisma.InputJsonValue,
        },
      });
    } catch (error) {
      this.logger.error(
