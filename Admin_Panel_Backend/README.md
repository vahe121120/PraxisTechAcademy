...
Deliberately a thin composition layer, not a parallel data-access
implementation — every endpoint here calls into `UsersService`,
`PaymentsService`, or `SubscriptionsService`, which already own that data
and its business rules. New capability was added to those services where
it was genuinely missing (student search, account suspend/reactivate,
per-user payment listing, manual subscription activation) rather than
...
|---|---|
| `totalStudents` | `User` count, `role: STUDENT`, excluding soft-deleted accounts |
| `activeSubscriptions` / `expiredSubscriptions` | `Subscription` counts by status |
| `monthlyRevenue` | **An array, not one number** — grouped by `Currency`, current calendar month, `status: SUCCEEDED` only. Summing AMD and USD into a single figure would be meaningless, so this returns `[{ currency, amount }, ...]`. `Payment` has neither an `amount` nor a `currency` column (both live on `Order` — see the schema note on why); this is joined via `Payment.order` and reduced in application code rather than `groupBy`, which can't aggregate across a relation |
| `popularCourses` | Top 5 by total historical enrollments. This is a two-hop aggregation (Course → CourseGroup → Enrollment) that Prisma's `groupBy` can't express — it only aggregates a model's own columns, not a multi-hop relation count — so it's one small, fixed (no user input, no injection surface), well-commented `$queryRaw` |

**Student management** (`/api/v1/admin/students`):

| Method | Path | Notes |
|---|---|---|
| GET | `/` | Search by name/email/phone (case-insensitive `contains`), optional status filter, paginated — scoped to `role: STUDENT` only, matching "student management" specifically |
| GET | `/:id` | 404s for a non-student id, not just a missing one — this resource's contract is students, not general user lookup |
| GET | `/:id/payments` | Every payment attempt, not just successful ones — a "why didn't my card work" support conversation needs the FAILED rows too |
| GET | `/:id/subscriptions` | Reuses `SubscriptionsService.findAllAdmin({ userId })` — already existed, no new method needed |
| POST | `/:id/suspend` | **Account-level** lockout (`User.status`) |
| POST | `/:id/reactivate` | Reverses the above |

**Manual activate/deactivate access** (`/api/v1/admin/subscriptions/:id/...`) —
kept as a *separate* resource from student suspend/reactivate above, on
purpose:

- **`suspend`/`reactivate`** (account-level): can this person log in at
  all. Deliberately does **not** cascade into cancelling their
  subscriptions — a suspension's blast radius should be exactly what the
  admin performing it intended, not an implicit side effect neither this
  spec nor common sense asked for.
- **`activate`/`deactivate`** (subscription-level): does this person have
  access to *this specific course* right now.
  - `POST /activate` — comps or manually restores access (a bank transfer
    handled outside ARCA, a goodwill extension, a support fix). Defaults
    `expireDate` to one month out if not given, and **reuses the exact
    same `TELEGRAM_ACCESS_PORT.grantAccess()` call a real successful
    payment triggers** — there must be exactly one code path that decides
    what "granting access" means, not a parallel "admin version" of it.
  - `POST /deactivate` — deliberately just delegates to the *already
    admin-callable* `SubscriptionsService.cancel()` rather than a second
    implementation of "revoke access." The route exists under `/admin/`
    for discoverability (an admin browsing admin endpoints shouldn't need
    to know that "deactivate" secretly means the student-facing `/cancel`
    route), not because the underlying logic needed duplicating.

**A real bug this pass found and fixed, not just a cosmetic note:** the
dashboard's monthly-revenue query was originally written as
`prisma.payment.groupBy({ by: ['currency'], _sum: { amount: true } })` —
but `Payment` has neither of those columns; both live on `Order`. That
would have failed at the database level the moment it ran against a real
schema (not caught here only because this sandbox can't generate a real
Prisma client to run it against at all). Rewritten to join through
`Payment.order` and reduce in application code, as described in the table
above.

**A second, smaller latent bug this pass also would have caught**,
consistent with the kind of check this codebase has done at every step:
`$queryRaw`'s result type doesn't resolve in this sandbox (the same
`@prisma/client` generation gap noted throughout this README), which would
otherwise have let `getPopularCourses`'s `.map()` callback silently fall
back to an implicit `any` parameter. It's now explicitly typed
(`PopularCourseRow`) rather than left to inference, so there's no
ambiguity once a real client exists.

### The connected payment → Telegram workflow, end to end

The 6-step "when ARCA payment succeeds" workflow was already functionally
wired from the previous two rounds (`PaymentsService` → `TELEGRAM_ACCESS_PORT`
→ `TelegramService`); this pass hardened it rather than rebuilding it, since
that's what an honest look at the existing code called for:

1. **Confirm payment** / **2. Create subscription** — `PaymentsService.applySuccessfulPayment()`, inside one DB transaction (billing state must be atomic).
3. **Find Telegram group** / 4. **Generate invite link** / 5. **Send Telegram message** / 6. **Save notification history** — `TelegramService.grantAccess()`, called *after* that transaction commits.

Each of the six steps now logs its own success/failure explicitly
(`[applySuccessfulPayment] Steps 1-2/6 OK...`, `[grantAccess] Step 4/6
OK...`), so a specific payment's grant can be traced to exactly where it
succeeded or stopped, rather than inferred from one opaque error at the
end — genuinely useful the first time a support ticket says "I paid but
never got the invite link."

**Two real gaps this pass fixed, not just logging cosmetics:**

- **Partial-failure handling for steps 4→5.** Previously, if invite-link
  generation (step 4) succeeded but sending the message (step 5) failed,
  the whole operation was caught by one broad `try/catch` and the
  already-created invite link was only visible by digging through
  `TelegramAccessGrant` rows by hand. Steps 4 and 5 now have separate
  `try/catch` blocks — a step-5 failure's notification record now includes
  the invite link and full message text that step 4 already produced, so
  support can hand it to the student directly from that one row instead of
  it being effectively lost.
- **Notification history was recording that something happened, not what.**
  `NotificationLog.metadata` previously stored only cross-reference ids
  (`courseGroupId`, `subscriptionId`). It now stores the actual invite link
  and the full welcome message text that was sent, plus a derived `subject`
  line (e.g. "Telegram group access granted — QA Automation") for a support
  agent scanning a student's history without opening every row.

**A latent bug this pass also caught and fixed**, unrelated to the workflow
logic itself: several `Prisma.InputJsonValue` writes across the codebase
(`AuditLog.before`/`after` in `AuthService`/`SubscriptionsService`,
`NotificationLog.metadata` in `TelegramService`/`SubscriptionsService`)
were passing a plain `Record<string, unknown>` without the same explicit
cast already used elsewhere (`payments.service.ts`'s `rawGatewayResponse`/
`payload` writes). `unknown` isn't structurally assignable to Prisma's
recursive JSON input type, so every one of these would have failed to
compile the moment a real, generated Prisma client resolved those types —
this sandbox's inability to generate the client had been silently masking
it. All six call sites now use the same `as unknown as Prisma.InputJsonValue`
