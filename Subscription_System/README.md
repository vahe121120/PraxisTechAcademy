...
**What's explicitly out of scope here** (flagged inline, not silently
skipped): granting Telegram group access and sending a confirmation
notification on successful payment — both are extension-point comments in
`applySuccessfulPayment()`, waiting on the `TelegramModule` and
`NotificationsModule` neither of which exist yet. Automatically expiring
stale `PENDING` orders (`OrdersService.expireStaleOrders()`) is implemented
but not wired to a cron — that belongs to the scheduler module described in
